...
class YTCFG_Plugin {
    ...
    // If true, whenever we successfully read Livestream Embedder settings and our settings are empty,
    // we'll copy them into our own option keys.
    ...
    public static function enqueue_assets() {
        ...
        .ytcfg-card{display:block;text-decoration:none}
        .ytcfg-thumb{width:100%;aspect-ratio:16/9;object-fit:cover;display:block;border-radius:10px}
        .ytcfg-title{margin:8px 0 0 0;font-size:14px;line-height:1.25;color:inherit}
    ...
    public static function admin_page() {
        ...
        <div class="wrap">
            ...
            <h2>Shortcode</h2>
            <p><code>[yt_channel_grid channel_id="UC..." include="term1|term2" exclude="term3|term4" match="any" max="24" cols="4" cache_minutes="120"]</code></p>
        </div>
        <?php
    }

    /**
     * Returns effective API key + default channel ID, preferring this plugin's settings,
     * falling back to Livestream Embedder settings if ours are blank.
     *
     * Also optionally mirrors Livestream Embedder settings into our settings when ours are empty.
     */
    private static function get_effective_settings() {
        ...
        // Optional one-way mirror (only if ours are empty and LS has values)
    ...
    public static function shortcode($atts) {
        $atts = shortcode_atts([
            'channel_id'     => '',
            'include'        => '',     // OR terms separated by |
            'exclude'        => '',     // OR terms separated by |
            'match'          => 'any',  // any|all
            'max'            => '24',
            'cols'           => '4',
            'cache_minutes'  => '120',
        ...
        $cache_minutes = max(1, min(10080, intval($atts['cache_minutes']))); // up to 7 days

        $include_terms = self::split_terms($atts['include']);
        $exclude_terms = self::split_terms($atts['exclude']);
        $match = strtolower(trim((string) $atts['match'])) === 'all' ? 'all' : 'any';
        $open_new_tab = intval($atts['open_new_tab']) === 1;
        ...
        if (is_array($cached)) {
            return self::render_grid($cached, $cols, $open_new_tab);
        }
        ...
        return self::render_grid($videos, $cols, $open_new_tab);
    }

    private static function split_terms($raw) {
        $raw = (string) $raw;
    ...
    private static function fetch_and_filter_playlist_videos($api_key, $playlist_id, $include_terms, $exclude_terms, $match, $max) {
        ...
        while (count($videos) < $max) {
            ...
            foreach ($items as $it) {
                ...
                $thumb = $sn['thumbnails']['medium']['url'] ?? ($sn['thumbnails']['high']['url'] ?? '');
                $videos[] = [
                    'video_id' => $video_id,
                    'title' => $title,
                    'thumb' => (string) $thumb,
    ...
    private static function render_grid($videos, $cols, $open_new_tab) {
        if (empty($videos)) {
        ...
        foreach ($videos as $v) {
            ...
            $href = esc_url('https://www.youtube.com/watch?v=' . $vid);

            $out .= '<a class="ytcfg-card" href="' . $href . '"' . $target . '>';
            if ($thumb !== '') {
